...
class NewEmployeeExport implements ShouldAutoSize, WithColumnWidths, WithEvents, WithStyles, WithTitle
{
    use Exportable;
    ...
    {
        $this->enum = $enum;
        $newEmployees = count($data) < 1 ? [] : $data->groupBy('employee.department.name')->flatten(1)->filter(function ($item) {
            if (!empty($this->enum['date_start']) && !empty($this->enum['date_end'])) {
                return $item->employee->date_to_company >= $this->enum['date_start'] || $item->employee->date_to_company <= $this->enum['date_end'];
            } else {
                return null;
            }
        });
        ...
        foreach ($newEmployees as $item) {
            $id = $item['employee_id'];
            // Kiểm tra xem id đã tồn tại trong mảng mới chưa
            if (isset($consolidatedItems[$id])) {
                // Cộng dồn salary nếu id đã tồn tại
                $consolidatedItems[$id]['gross_salary'] += $item['gross_salary'];
            } else {
                // Nếu chưa có, thêm item này vào mảng mới và sử dụng id làm key
                $consolidatedItems[$id] = $item;
            }
        }
        $this->data = $consolidatedItems;
    }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $text = !empty($this->enum['date_start']) && !empty($this->enum['date_end']) ?
            Carbon::parse($this->enum['date_start'])->format('d-M-Y').' - '.Carbon::parse($this->enum['date_end'])->format('d-M-Y') : '';
        $sheet->getCell('C2')->setValue('NEW STAFF RECORDS DURING:'.$text);
